...
const distributeGrandPoint = async (buyerId, grandPoint, buyerphone, grandTotalPrice) => {
  ...
  for (const upline of flatUplines) {
    ...
    if (found && upline.GenerationLevel >= upline.level) {
      eligibleUplines.push({ ...upline });
    }
  }

  const totalEligible = eligibleUplines.length;
  if (totalEligible > 0) {
    const pointPerUpline = thirtyPercent / totalEligible;
    // console.log(`🧮 Total ${totalEligible} eligible users. Each will receive ${pointPerUpline} points.`);
    ...
    for (const upline of eligibleUplines) {
      ...
      uplineUser.AllEntry.incoming.push({
        ...
        sector: `Shared Generation Commission (${upline.level})`,
        date: new Date()
      });

      await uplineUser.save();
      // console.log(`✅ ${uplineUser.name} received ${pointPerUpline} points`);
    }
